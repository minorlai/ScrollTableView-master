...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
    ...
    public void setData(List<ImageView> views, List<ADInfo> list, ImageCycleViewListener listener, int showPosition) {
        ...
        // 设置指示器
        indicators = new View[ivSize];
        if (isCycle)
            indicators = new View[ivSize - 2];
        indicatorLayout.removeAllViews();
        for (int i = 0; i < indicators.length; i++) {
            View view = LayoutInflater.from(getActivity()).inflate(
                    R.layout.view_cycle_viewpager_indicator, null);
            indicators[i] = (View) view.findViewById(R.id.image_indicator);
        ...
        viewPager.setOffscreenPageLimit(3);//缓存数量3
    ...
    public void setIndicatorCenter() {
        RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
                ...
                RelativeLayout.LayoutParams.WRAP_CONTENT);
        params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);//底部放置
        params.addRule(RelativeLayout.CENTER_HORIZONTAL);//小圆点的布局方式（水平居中）
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i]
                    .setBackgroundResource(R.mipmap.indicator_nor);
        }
        if (indicators.length > selectedPosition)
            indicators[selectedPosition]
                    .setBackgroundResource(R.mipmap.indicator_sl);
    }
